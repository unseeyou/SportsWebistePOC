...
def summarise_sport(sport_name: str):
    ...
    cursor.execute("SELECT attendance FROM attendance_records WHERE activity = ? collate NOCASE", (sport_name,))
    ...
    fig = px.pie(
        ...
        hole=0.5, names=[i for i in formatted_data],
...
def summarise_sport_individual(sport_name: str, student_id: int):
    sport_name = sport_name.strip()
    cursor: sqlite3.Cursor = app.database.get_cursor()
    cursor.execute("SELECT student_id, attendance FROM attendance_records WHERE activity = ? collate NOCASE AND student_id = ?", (sport_name, student_id))
    data = cursor.fetchall()
